# TidyUUUUp v1.2.0
> **Liquid Glass Edition**：保留 v1.1.2 全部功能，UI 全面升级为苹果 Liquid Glass 设计语言——更通透的亚克力基底、顶部镜面高光、折射边缘边框、深度渐变。
...
v1.2.0 仅重写 UI 绘制层，所有功能逻辑与 v1.1.2 完全一致。

| UI 元素 | Liquid Glass 改进 |
| --- | --- |
| 主小岛背景 | 垂直深度渐变（顶部亮、底部暗），alpha 从 208 降至 119，更通透 |
| 顶部高光 | 白色镜面渐变带，模拟玻璃表面反光 |
| 边缘边框 | 外层折射高光（alpha 55）+ 内层微妙边框，双层玻璃感 |
| 底部内阴影 | 深色渐变，增加悬浮深度 |
| 弹出面板 | 同步升级为 Liquid Glass 材质，更通透的背景 + 顶部高光 + 双层边框 |
| 文件夹图标 | 玻璃质感蓝色渐变 + 顶部高光 + 折射边缘，悬停时更亮 |
| 搜索框 | 更透明的背景，聚焦时蓝色光晕边框 |
| 固定按钮 | 玻璃质感半透明背景，悬停高亮 |
| 系统亚克力 | `AccentPolicy` 颜色调整为蓝灰色调（`0x771E2230`），与 Liquid Glass 协调 |
...
从 GitHub Release 下载 `TidyUUUUp_Setup_v1.2.0.exe` 双击安装。安装程序只替换应用目录，`%APPDATA%/TidyUUUUp/settings.json` 和用户数据保持不变。
