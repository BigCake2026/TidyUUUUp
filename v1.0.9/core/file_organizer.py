    ...
    def organize_directory(self, source_dir, target_dir=None, strategy='category', use_subfolders=True, dry_run=False):
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            for filename in files:
                ...
                # 跳过目标目录中的文件 (用路径前缀匹配, 避免 in 子串匹配误判:
                # 例如 target_dir="/home/user/Doc" 不应匹配 "/home/user/Documents")
                if target_dir and source_dir != target_dir:
                    # 统一用 os.sep 后缀确保前缀匹配整个路径段
                    if filepath.startswith(target_dir.rstrip(os.sep) + os.sep) or filepath == target_dir:
                        continue
