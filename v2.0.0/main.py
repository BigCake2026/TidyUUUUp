...
class TidyDynamicIslandDock(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.search_input.setStyleSheet("""
            ...
            QLineEdit:hover { background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.28); }
            QLineEdit:focus { background: rgba(255,255,255,0.14); border: 1.5px solid rgba(10,132,255,0.9); }
...
def application_icon() -> QIcon:
    ...
    # Liquid Glass 图标：蓝色深度渐变 + 顶部高光 + 玻璃边缘
    body = QPainterPath()
    body.addRoundedRect(4, 4, 56, 56, 18, 18)
    grad = QLinearGradient(0, 4, 0, 60)
    grad.setColorAt(0.0, QColor(64, 168, 255, 250))
    grad.setColorAt(0.55, QColor(20, 132, 255, 252))
    grad.setColorAt(1.0, QColor(0, 88, 220, 255))
    painter.fillPath(body, QBrush(grad))
    # 顶部镜面高光
    hl = QLinearGradient(0, 6, 0, 30)
    hl.setColorAt(0.0, QColor(255, 255, 255, 90))
    hl.setColorAt(1.0, QColor(255, 255, 255, 0))
    painter.fillPath(body, QBrush(hl))
    # 折射边缘
    painter.setPen(QPen(QColor(255, 255, 255, 70), 1.5))
    painter.drawPath(body)
    # 白色 Tidy 标识
    painter.setPen(QPen(QColor(255, 255, 255, 235), 7, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
    painter.drawLine(24, 26, 40, 26)
    painter.drawLine(32, 26, 32, 40)
    painter.end()
    return QIcon(pixmap)
