...
from PyQt6.QtCore import QDate, QEvent, QPoint, QPropertyAnimation, QStandardPaths, QThread, QTime, QTimer, Qt, QEasingCurve, pyqtProperty, pyqtSignal
...
class AppleVectorFolderIcon(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        if self.hovered:
            ...
            painter.drawRoundedRect(1, 1, 34, 34, 10, 10)
        tab = QPainterPath()
        tab.addRoundedRect(4, 6, 14, 8, 3, 3)
        painter.fillPath(tab, QColor(0, 102, 204))
        body = QPainterPath()
        body.addRoundedRect(3, 10, 30, 20, 5, 5)
        ...
        painter.setPen(QPen(QColor(255, 255, 255, 100 if self.hovered else 60), 1))
...
class TidyDynamicIslandDock(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.search_input.setStyleSheet("""
            QLineEdit { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); border-radius: 14px; padding: 0 12px; color: #FFFFFF; font-family: 'Segoe UI'; font-size: 11px; }
            QLineEdit:focus { background: rgba(255,255,255,0.14); border: 1px solid #0A84FF; }
        """)
    ...
    def create_pinned_button(self, label: str, tooltip: str) -> QPushButton:
        ...
        button.setStyleSheet("""
            QPushButton { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10); border-radius: 8px; color: #FFFFFF; font-size: 14px; font-family: 'Segoe UI Symbol'; }
    ...
    def paintEvent(self, event) -> None:
        ...
        painter.fillPath(body, QColor(24, 24, 26, 208))
