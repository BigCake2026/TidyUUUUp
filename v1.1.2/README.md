...
更新检查是应用的固定能力，**用户不能关闭自动检查，也不能跳过某个版本**。每次启动时，应用会在后台读取公开 GitHub Release 的最新版本；若发现更高版本，会优先下载 `Setup EXE` 安装程序，并由用户点击“安装更新”后执行。安装程序只替换当前用户应用目录中的应用文件；`%APPDATA%/TidyUUUUp/settings.json`、桌面索引路径、小岛位置和快捷方式记录保持不变。下载期间同样只写入用户下载目录。
