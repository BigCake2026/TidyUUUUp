...
class UpdateDialog(QDialog):
    ...
    def __init__(self, update_info, parent=None):
        ...
        # 用成员定时器替代 singleShot(lambda),
        # 随对象销毁自动清理, 避免对话框关闭后延迟回调命中已销毁对象
        self._apply_timer = QTimer(self)
        self._apply_timer.setSingleShot(True)
        self._apply_timer.timeout.connect(self._do_apply_update)
        self._pending_update_file = None
    ...
    def _on_download_finished(self, success, result):
        ...
        # 应用更新 - 用成员定时器, 对话框关闭时会自动停止, 避免回调命中已销毁对象
        self._pending_update_file = result
        self._apply_timer.start(300)

    def _do_apply_update(self):
        """成员定时器回调 - 安全的应用更新"""
        if self._pending_update_file is None:
            return
        update_file = self._pending_update_file
        self._pending_update_file = None
