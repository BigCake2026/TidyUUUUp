"""Headless smoke tests for TidyUUUUp v1.1.0.
...
def main() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert dock.width() == dock.compact_width
        assert dock.layout().count() == 3, "The centered layout must retain left/right balancing spacers."
        dock.expand_search()
        assert dock.search_width_animation.endValue() == 270
        assert dock.width_animation.endValue() == dock.stretched_width
        dock.search_input.setText("main")
        dock.run_search()
        assert dock.popover.title.text() == "搜索：main"
